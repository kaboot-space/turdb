...
// TestBasicTableOperationsFixed tests basic table creation, insertion, and data verification with proper initialization
func TestBasicTableOperationsFixed(t *testing.T) {
	// Create temporary file for testing
	tempFile := "/tmp/test_basic_operations_fixed.turdb"
	...
	// CRITICAL: Initialize the file format first
	err = fileFormat.Initialize(4096) // 4KB page size
	if err != nil {
		t.Fatalf("Failed to initialize file format: %v", err)
	}

	// CRITICAL: Map the file to memory after initialization
	err = fileFormat.Open()
	if err != nil {
